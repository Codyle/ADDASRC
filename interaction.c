...
#define SET_FUNC_POINTERS(type,name) { type##_int = &type##_##name##_int; type##_real = &type##_##name##_real; }
	...
	switch (IntRelation) {
		case G_POINT_DIP:
			SET_FUNC_POINTERS(InterTerm, poi);
			break;
		case G_FCD:
			SET_FUNC_POINTERS(InterTerm, fcd);
			break;
		case G_FCD_ST:
			SET_FUNC_POINTERS(InterTerm, fcd_st);
			break;
		case G_IGT_SO:
			...
			break;
		case G_NLOC:
			SET_FUNC_POINTERS(InterTerm, nloc);
			break;
		case G_NLOC_AV:
			SET_FUNC_POINTERS(InterTerm, nloc_av);
			break;
		case G_SO:
		 ...
		 */
		default:
			LogError(ONE_POS, "Invalid interaction term calculation method: %d", (int)IntRelation);
			// no break
	}
	...
	if (surface) {
		...
		switch (ReflRelation) {
			case GR_IMG:
				SET_FUNC_POINTERS(ReflTerm, img);
				break;
			case GR_SOM:
			 ...
			 */
			default:
				LogError(ONE_POS, "Invalid reflection term calculation method: %d", (int)ReflRelation);
				// no break
		}
