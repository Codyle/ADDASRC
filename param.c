	...
	if (IFROOT) {
		...
		switch (PolRelation) {
			case POL_CLDR:
				...
				break;
			case POL_CM:
				fprintf(logfile, "'Clausius-Mossotti'\n");
				break;
			case POL_DGF:
				fprintf(logfile, "'Digitized Green's Function'\n");
				break;
			case POL_FCD:
				fprintf(logfile, "'Filtered Coupled Dipoles'\n");
				break;
			case POL_IGT_SO:
				fprintf(logfile, "'Integration of Green's Tensor [approximation O(kd^2)]'\n");
				break;
			case POL_LAK:
				fprintf(logfile, "'by Lakhtakia'\n");
				break;
			case POL_LDR:
			...
			case POL_NLOC_AV:
				...
				break;
			case POL_RRC:
				fprintf(logfile, "'Radiative Reaction Correction'\n");
				break;
			case POL_SO:
				fprintf(logfile, "'Second Order'\n");
				break;
		}
		/* TO ADD NEW POLARIZABILITY FORMULATION
		...
		switch (ScatRelation) {
			case SQ_DRAINE:
				fprintf(logfile, "'by Draine'\n");
				break;
			case SQ_FINDIP:
				fprintf(logfile, "'Finite Dipoles'\n");
				break;
			case SQ_IGT_SO:
				fprintf(logfile, "'Integration of Green's Tensor [approximation O(kd^2)]'\n");
				break;
		...
		switch (IntRelation) {
			case G_FCD:
				fprintf(logfile, "'Filtered Green's tensor'\n");
				break;
			case G_FCD_ST:
			...
			case G_IGT_SO:
				...
				break;
			case G_NLOC:
				fprintf(logfile, "'Non-local' (point-value, Gaussian width Rp="GFORMDEF")\n", nloc_Rp);
				break;
			case G_NLOC_AV:
				fprintf(logfile, "'Non-local' (averaged, Gaussian width Rp="GFORMDEF")\n", nloc_Rp);
				break;
			case G_POINT_DIP:
				fprintf(logfile, "'as Point dipoles'\n");
				break;
			case G_SO:
				fprintf(logfile, "'Second Order'\n");
				break;
		...
		if (surface) {
			...
			switch (ReflRelation) {
				case GR_IMG:
					fprintf(logfile, "'Image-dipole approximation'\n");
					break;
				case GR_SOM:
					fprintf(logfile, "'Sommerfeld integrals'\n");
					break;
			}
		}
		/* TO ADD NEW REFLECTION FORMULATION
...
#	endif
		...
		switch (IterMethod) {
			case IT_BCGS2:
				fprintf(logfile, "Enhanced Bi-CG Stabilized(2)\n");
				break;
			case IT_BICG_CS:
				fprintf(logfile, "Bi-CG (complex symmetric)\n");
				break;
			case IT_BICGSTAB:
				fprintf(logfile, "Bi-CG Stabilized\n");
				break;
			case IT_CGNR:
				fprintf(logfile, "CGNR\n");
				break;
			case IT_CSYM:
				fprintf(logfile, "CSYM\n");
				break;
			case IT_QMR_CS:
				fprintf(logfile, "QMR (complex symmetric)\n");
				break;
			case IT_QMR_CS_2:
				fprintf(logfile, "2-term QMR (complex symmetric)\n");
				break;
		}
		/* TO ADD NEW ITERATIVE SOLVER
		 * add a case above in the alphabetical order, analogous to the ones already present. The variable parts of the
		...
		switch (sym_type) {
			case SYM_AUTO:
				...
				break;
			case SYM_NO:
				fprintf(logfile, "Symmetries: cancelled by user\n");
				break;
			case SYM_ENF:
				fprintf(logfile, "Symmetries: enforced by user (warning!)\n");
				break;
		}
		...
		if (chp_type != CHP_NONE) {
			...
			switch (chp_type) {
				case CHP_NONE:
					break; // redundant; to keep a set of cases complete
				case CHP_NORMAL:
					fprintf(logfile, "    type = normal\n");
					break;
				case CHP_REGULAR:
					fprintf(logfile, "    type = regular\n");
					break;
				case CHP_ALWAYS:
					fprintf(logfile, "    type = always\n");
					break;
			}
