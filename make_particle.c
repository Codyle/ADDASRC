	...
	if (save_geom_fname[0] == 0) {
		...
		switch (sg_format) {
			...
			case SF_DDSCAT7:
				...
				break;
			default:
				LogError(ONE_POS, "Unknown format for saved geometry file (%d)", (int)sg_format);
				// no break
		}
 ...
 */
{
	...
	while (FGetsError(dipfile, fname, &line, linebuf, BUF_LINE, ONE_POS) != NULL) {
		...
		switch (read_format) {
			case SF_TEXT:
				scanned = sscanf(linebuf, geom_format, &x, &y, &z);
				break;
			case SF_TEXT_EXT:
				scanned = sscanf(linebuf, geom_format_ext, &x, &y, &z, &mat);
				break;
			case SF_DDSCAT6:
 ...
 */
{
	...
	while (fgets(linebuf, BUF_LINE, dipfile) != NULL) {
		...
		switch (read_format) {
			case SF_TEXT:
				scanned = sscanf(linebuf, geom_format, &x0, &y0, &z0);
				break;
			case SF_TEXT_EXT:
				scanned = sscanf(linebuf, geom_format_ext, &x0, &y0, &z0, &mat);
				break;
			case SF_DDSCAT6:
			case SF_DDSCAT7:
				scanned = sscanf(linebuf, ddscat_format_read2, &x0, &y0, &z0, &mat);
				break;
		}
		/* TO ADD NEW FORMAT OF SHAPE FILE
	...
	for (k = local_z0; k < local_z1_coer; k++) for (j = 0; j < boxY; j++) for (i = 0; i < boxX; i++) {
				...
				switch (shape) {
					...
					case SH_RBC:
						...
						break;
					case SH_READ:
						break; // just to have a complete set of cases; this cases is treated separately below
					case SH_SPHERE:
